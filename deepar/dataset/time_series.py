...
class NasaTs(Dataset):
    def __init__(self, s = None, n_steps = 20):
    ...
    def s2df(self):
        df = pd.DataFrame()
        for i in range(len(self.s)-(self.n_steps+1)):
            df[str(i)] = self.s.values[i:i+(self.n_steps+1)]
        df.to_csv("temp/ts_array.csv",index = False)
        df = df.T
        return df
    
    def next_batch(self, batch_size, n_steps):
        groups_list = np.arange(0,len(self.data))
        np.random.shuffle(groups_list)
        selected_groups = groups_list[:batch_size]
        input_data = self.data.iloc[selected_groups,:]
        
        input_data = input_data.values
        return input_data[:, :-1].reshape(-1, n_steps, 1), input_data[:, 1:].reshape(-1, n_steps, 1)
    
    def get_one_batch(self,batch_num,n_steps):
        input_data = self.data.iloc[batch_num,:]
        input_data = input_data.values
        return input_data[:, :-1].reshape(-1, n_steps, 1), input_data[:, 1:].reshape(-1, n_steps, 1)
...
class MockTs(Dataset):
    ...
    def next_batch(self, batch_size, n_steps):
        ...
        """
        t0 = np.random.rand(batch_size, 1) * (self.t_max - self.t_min - n_steps * self.resolution)  # t0 size (batch_size, 1)
        Ts = t0 + np.arange(0., n_steps + 1) * self.resolution
        ys = self._time_series(Ts)  # _time_series是两个sin函数的组合
        return ys[:, :-1].reshape(-1, n_steps, 1), ys[:, 1:].reshape(-1, n_steps, 1)  # [input, output] input_size = output_sise = [batch, step]
    @property
    def mock_ts(self):
        """
...
#     def __init__(self, pandas_df, grouping_variable='category'):
#         super().__init__()
#         self.data = pandas_df
...
#                    padding_value=0):
#         """
#         :param batch_size: how many time series to be sampled in this batch (int) # 训练时用多少组数据
#         :param n_steps: how many RNN cells (int) 用多少个数据预测下一个数据
